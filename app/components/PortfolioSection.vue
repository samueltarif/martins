<template>
  <section id="portfolio" class="py-16 bg-[var(--bg-secondary)]">
    <div class="max-w-7xl mx-auto px-5">
      <h2 class="text-4xl font-bold text-center mb-2 text-[var(--text-primary)]">Portfólio de Soluções</h2>
      <p class="text-center text-[var(--text-primary)] mb-10 text-lg">Produtos e serviços de alta qualidade para transformar seu negócio</p>
      <div class="flex flex-wrap justify-center gap-3 mb-10">
        <button v-for="cat in categories" :key="cat.key" @click="setFilter(cat.key)" class="px-6 py-2 rounded-full bg-black text-white font-medium">{{ cat.label }}</button>
      </div>
