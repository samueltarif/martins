<template>
  <section id="about" class="py-16" ref="sectionRef">
    <div class="max-w-7xl mx-auto px-5">
      <h2 class="text-4xl font-bold text-center mb-2 bg-gradient-to-r from-[var(--accent-primary)] to-[var(--accent-secondary)] bg-clip-text text-transparent">Sobre Mim</h2>
      <p class="text-center text-[var(--text-secondary)] mb-12 text-lg">Especialista em vendas B2B com foco em tecnologia e soluções corporativas</p>
      <div class="grid lg:grid-cols-3 gap-10 items-center">
        <div class="lg:col-span-1">
          <img :src="aboutImg" alt="Fellip Martins - Foto profissional" class="w-full rounded-2xl shadow-xl border-4 border-[var(--accent-primary)]" loading="lazy" />
        </div>
        <div class="lg:col-span-2">
          <h3 class="text-xl font-semibold mb-3">Experiência Comprovada em Vendas</h3>
          <p class="text-[var(--text-secondary)] mb-6 leading-relaxed">Com mais de 5 anos de experiência no mercado B2B, atuo oferecendo soluções industriais completas — incluindo tubos, flanges, conexões, válvulas e instrumentação — sempre com foco em qualidade, certificação e alta performance.</p>
          ...
          <div class="grid sm:grid-cols-2 lg:grid-cols-4 gap-6 mt-6">
            <div class="text-center p-6 bg-[var(--bg-secondary)] rounded-2xl border border-black/10 shadow">
              <span class="text-3xl font-extrabold bg-gradient-to-r from-[var(--accent-primary)] to-[var(--accent-secondary)] bg-clip-text text-transparent block mb-2">{{ clientsDisplay }}</span>
              <span class="text-[var(--text-secondary)] text-sm font-medium">Clientes Ativos</span>
            </div>
            <div class="text-center p-6 bg-[var(--bg-secondary)] rounded-2xl border border-black/10 shadow">
              <span class="text-3xl font-extrabold bg-gradient-to-r from-[var(--accent-primary)] to-[var(--accent-secondary)] bg-clip-text text-transparent block mb-2">{{ yearsDisplay }}</span>
              <span class="text-[var(--text-secondary)] text-sm font-medium">Anos de Experiência</span>
            </div>
            <div class="text-center p-6 bg-[var(--bg-secondary)] rounded-2xl border border-black/10 shadow">
              <span class="text-3xl font-extrabold bg-gradient-to-r from-[var(--accent-primary)] to-[var(--accent-secondary)] bg-clip-text text-transparent block mb-2">{{ projectsDisplay }}</span>
              <span class="text-[var(--text-secondary)] text-sm font-medium">Projetos Concluídos</span>
            </div>
            <div class="text-center p-6 bg-[var(--bg-secondary)] rounded-2xl border border-black/10 shadow">
              <span class="text-3xl font-extrabold bg-gradient-to-r from-[var(--accent-primary)] to-[var(--accent-secondary)] bg-clip-text text-transparent block mb-2">{{ satisfactionDisplay }}%</span>
